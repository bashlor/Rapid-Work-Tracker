...
import { Clock, CalendarIcon, Save, Play, Pause, Square, Undo2 } from 'lucide-react'
import { usePage, Link } from '@inertiajs/react'
import React, { useEffect, useState } from 'react'
import { cn } from '@/lib/utils'
import { getTitleByPath, getRouteByPath } from '@/lib/routes'
...
const AppHeader = ({ children }: AppHeaderProps) => {
  const { state, formatElapsedTime, pauseTimer, resumeTimer, stopTimer } = useTimer()
    ...
    <div className="border-b border-border h-16 p-4 bg-white">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          {currentRoute?.icon && <currentRoute.icon size={24} className="text-primary" />}
